#!/usr/bin/env bash
#
# Create a self-signed code-signing certificate for flow-bar so the macOS
# Automation (TCC) grant persists across rebuilds and `brew upgrade`.
#
# Why: ad-hoc signing (`codesign -s -`) produces a fresh signature every build,
# so macOS treats each rebuild as a new app and drops the grant. flow-bar needs
# that grant: FlowClient.spawnDisclaimed deliberately does NOT disclaim
# responsibility for the AppleScript terminal backends (iterm/terminal/warp/
# ghostty), so flow-bar itself owns the "wants to control <app>" permission.
# Lose it and `flow do` starts failing with -1743 for no visible reason.
#
# A stable signing identity gives the app a fixed Designated Requirement, which
# TCC keys the grant to — grant once, and it sticks across every rebuild.
#
...
# self-signed) — that's fine, exactly like ad-hoc. Trust is only needed to
# *validate* a signature, not to produce one.
#
# The keychain password is hardcoded on purpose. It protects nothing but local
# code-identity continuity — there is no secret here worth hiding, and making it
# interactive would break `brew install`. Please don't "fix" it.
#
# Idempotent: re-running reuses the existing identity if present.
#
# Teardown:
#   security delete-keychain "$HOME/Library/Keychains/flow-bar-signing.keychain-db"
#
set -euo pipefail

IDENTITY_NAME="${CODESIGN_IDENTITY:-flow-bar Code Signing}"
...
#
# NOT `-v`: a self-signed codesigning cert always reports CSSMERR_TP_NOT_TRUSTED,
# so the valid-only listing hides it even though `codesign` signs with it fine.
# Using `-v` here would make us think the cert is missing and destructively
# recreate it — minting a *new* leaf, changing the DR, and losing the grant.
...
# Grant codesign non-interactive access to the private key. This is the piece
# that suppresses the GUI "codesign wants to access key ..." dialog.
